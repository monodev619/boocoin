...
#if defined(NDEBUG)
# error "Boocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0a8e8520dbc5fbeedbaf0940c562bb3f6fbe17830b4bcb80ee9dce3c9fe327e1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Boocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Boocoin Signed Message:\n";
...
{
    // Boocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Boocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 24 * 60 * 60; // Boocoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Boocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Boocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Boocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x38be08e651aefd6f6717b0a0b872b94e8ce1a4386548ee21bbfd095cab413e10");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1524033251;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084652120;

        if (fTestNet)
        {
            block.nTime    = 1524033222;
            block.nNonce   = 385540189;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
{
	...
	{
#if defined(USE_SSE2)
		// Detection would work, but in cases wehre we KNOW it always has sse2,
		// it is faster to use directly than to use a function pointer or conditional.
#if defined(_M_X64) || defined(_M_AMD64) || (defined(MAC_OSX) && defined(__i386__))
		// Always SSE2: x86_64 or Intel MacOS X
		scrypt_1024_1_1_256_sp_sse2(BEGIN(block.nVersion), BEGIN(thash), scratchpad);
#else
		// Detect SSE: 32bit x86 Linux or Windows
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Boocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BoocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BoocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BoocoinMiner(CWallet *pwallet)
{
    printf("BoocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("boocoin-miner");
    ...
    try { loop {
        ...
        printf("Running BoocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BoocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BoocoinMiner, pwallet));
}
